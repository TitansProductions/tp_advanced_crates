...
function closeInventory() {
    $.post("http://tp-advancedcrates/closeNUI", JSON.stringify({}));
...
$(function() {
	window.addEventListener('message', function(event) {
		if (event.data.type == "enable_inventory_crate") {
			document.body.style.display = event.data.enable ? "block" : "none";

			document.getElementById("showloading").style.display="none";

			document.getElementById("crate_inventory").style.display="block";
			...
			document.getElementById("crate_inventory").style.display="none";

			document.getElementById("showloading").style.display="block";

			canClickButton = false;

        }else if (event.data.action == "addCrateInformation") {

			document.getElementById("otherPlayerInventoryHeader").innerHTML = "Crate Inventory Contents";
			
		}else if (event.data.action == "nodrag") {
            $('.item').draggable("disable");
			$('.otherItem').draggable("disable");

		} else if (event.data.action == "setCrateItems") {
...
$(document).ready(function () {
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            $.post("http://tp-advancedcrates/TakeFromCrate", JSON.stringify({
                item: itemData,
